...
- Mobile dev server (Expo Go preview): a second, separate Oracle Cloud VM (Oracle Linux, `opc` user) runs `npx expo start --host lan` as a `systemd` service (`ipo-mobile-dev`), purely so in-progress/unreleased mobile code can be tested live in Expo Go before it's built into a real app — it plays no role in the actual OTA/production pipeline above. `.github/workflows/deploy-dev-server.yml` SSHs in on every `mobile/**` push and does `git pull` + `npm install` + `systemctl restart ipo-mobile-dev` to keep it in sync with `main`. Deliberately isolated from the production backend VM (which is memory-constrained) to avoid resource contention.
